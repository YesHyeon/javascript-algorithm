...
function solution(num1, num2, arr) {
  ...
  for (let i = 0; i < length; i++) {
    for (let j = i + 1; j < length; j++) {
      for (let q = j + 1; q < length; q++) {
        sumArray.push(arr[i] + arr[j] + arr[q]);
      }
    }
  }
  let sumArrayUnique = sumArray.filter((v, i) => sumArray.indexOf(v) === i);

  sumArrayUnique.sort((a, b) => {
    if (a < b) return 1;
    if (a == b) return 0;
    if (a > b) return -1;
  });
  answer = sumArrayUnique[num2 - 1];
  return answer;
}
